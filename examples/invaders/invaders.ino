...
char s[32]; // general string buffer
bool scored = false;
...
bool writeWithDelay(const char **str, byte x, byte y, byte start, byte end, int delay)
{
  byte i;
  for(i=start;i<=end;i++) {
...
bool titleScreen() {
  ...
  if (writeWithDelay(&strings[2], 28, 10, 1, 17, d))
    return true;

  if (writeWithDelay(&strings[1], 56, 20, 1, 3, d))
    return true;
  ...
  if (writeWithDelay(&strings[4], 20, 85, 1, 24, d))
    return true;

  // Explosions
  explosionSound();
  drawBitmap(24, 20, BITMAP_EXPLOSION);
  if (pollFireButton(d*3)) {
     return true;
  }

  explosionSound();
  drawBitmap(44, 45, BITMAP_EXPLOSION);
  if (pollFireButton(d*3)) {
     return true;
  }

  explosionSound();
  drawBitmap(80, 40, BITMAP_EXPLOSION);
  if (pollFireButton(d*3)) {
     return true;
  }

  explosionSound();
  drawBitmap(10, 40, BITMAP_EXPLOSION);
  if (pollFireButton(d*3)) {
     return true;
  }
...
void advanceInvaders() {
  ++invaderTypeToggle;  // toggle between 0 and 1
  invaderTypeToggle = invaderTypeToggle % 2;
...
void drawBitmap(byte x, byte y, unsigned int bitmapIndex) {
  const uint16_t PROGMEM *index = bitmaps + bitmapIndex;
  uint16_t ii;
  byte hi, lo;
  byte pixelOffset;
  int byteIndex;
  ...
  byteIndex = (y*(W/8)) + (x/8);
  pixelOffset = x % 8;
  bool nowrap1 = ((byteIndex+1) % (W/8)) != 0;
  bool nowrap2 = ((byteIndex+2) % (W/8)) != 0;
