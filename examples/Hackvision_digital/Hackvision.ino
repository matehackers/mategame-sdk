...
const uint16_t PROGMEM bitmaps[] = { 
#include "bitmaps.h"
};

const char PROGMEM s0[] = "HIGH SCORES";
const char PROGMEM s1[] = "PLA";
const char PROGMEM s2[] = "SPACE INVADERS";
const char PROGMEM s3[] = " =  10";
const char PROGMEM s4[] = " =  20";
const char PROGMEM s5[] = " =  30";
const char PROGMEM s6[] = " =  ??";
const char PROGMEM s7[] = "GAME";
const char PROGMEM s8[] = "OVER";
const char PROGMEM s9[] = "PONG";

const char* const PROGMEM strings[] = {s0, s1, s2, s3, s4, s5, s6, s7, s8, s9};
...
// 0 is the default unused pin
ButtonController gamepad1(3, 4, 5, 6, 1), gamepad2(0, 0, 10, 11, 2);
...
void pong() {
  ...
  if (dy == 0) {
    ...
    if (PaddleA.buttonPressed() || gamepad1.firePressed()) {
      if (random(0, 2) == 0) {
        dy = 1;
      } else {
        dy = -1;
    ...
    if (PaddleB.buttonPressed() || gamepad2.firePressed()) {
      if (random(0, 2) == 0) {
        dy = 1;
      } else {
        dy = -1;
...
void drawPaddles() {
  ...
  if ((gamepad1.upPressed()) && (paddleAy > 1)) {
    paddleAy = paddleAy - 2;
  }
  if ((gamepad1.downPressed()) && (paddleAy < (H-paddleLength-1))) {
    paddleAy = paddleAy + 2;
  }

  if ((gamepad2.upPressed()) && (paddleBy > 1)) {
    paddleBy = paddleBy - 2;
  }
  if ((gamepad2.downPressed()) && (paddleBy < (H-paddleLength-1))) {
    paddleBy = paddleBy + 2;
  }
...
boolean getInput() {
  ...
  if (((gamepad1.leftPressed()) ) && (cannonX > 0)) {
    oldCannonX = cannonX;
    cannonXF--;
    ...
    if (((gamepad1.rightPressed()) ) && (cannonX < (W-CANNON_WIDTH))) {
      oldCannonX = cannonX;
      cannonXF++;
      cannonX = (cannonXF + 5) / 100;
      if (oldCannonX != cannonX) {
